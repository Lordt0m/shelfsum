...
- **Next safe action:** implement slice 2 test-first: add an explicit PostgreSQL test environment and a GitHub Actions service-database workflow that proves the three PostgreSQL-only transaction/concurrency checks execute rather than skip, while retaining the documented SQLite fast loop.
