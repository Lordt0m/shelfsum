...
from catalogue.services import ProductCreation, ProductUpdate
...
class ProductCatalogueForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        matches = Product.objects.filter(business=self.business, name__iexact=name)
        if self.instance.pk:
    ...
    def clean_sku(self):
        ...
        matches = Product.objects.filter(business=self.business, sku__iexact=sku)
        if self.instance.pk:
            matches = matches.exclude(pk=self.instance.pk)
        if sku and matches.exists():
            raise forms.ValidationError("A Product with this SKU already exists.")
        return sku


class ProductCreationForm(ProductCatalogueForm):
    ...
    class Meta(ProductCatalogueForm.Meta):
        fields = (
            "name",
            "sku",
            "description",
...
class ProductUpdateForm(ProductCatalogueForm):
    def to_product_update(self):
